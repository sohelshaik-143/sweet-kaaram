...
function renderOrders(newOrderId = null) {
  ...
  currentOrders.slice().reverse().forEach((order, index) => {
    ...
    const itemsList = Array.isArray(order.items) && order.items.length
      ...
      : '-';
    const qtyTotal = Array.isArray(order.items) ? order.items.reduce((sum,i)=>sum+i.qty,0) : 0;
    const totalAmount = order.totalAmount ?? (Array.isArray(order.items) ? order.items.reduce((sum,i)=>sum+(i.qty*i.price),0) : 0);
    ...
    let actionBtn = '';
    if (status.toLowerCase() === 'pending') actionBtn = `<button onclick="updateStatus('${order['Tracking ID']}','Out for Delivery')" class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 transition">Out for Delivery</button>`;
    else if (status.toLowerCase() === 'out for delivery') actionBtn = `<button onclick="updateStatus('${order['Tracking ID']}','Delivered')" class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition">Mark Delivered</button>`;
...
document.getElementById('addOrderForm').addEventListener('submit', async e=>{
  ...
  try{
    ...
    const data=await res.json();
    if(data.success){ alert(`✅ Order Placed! Tracking ID: ${data.orderId}`); document.getElementById('addOrderForm').reset(); }
